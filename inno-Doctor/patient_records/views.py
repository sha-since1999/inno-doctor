...
def eprescriptionList(request, id):
    ...
    try:
        ...
        return render(
                request, "patient_records/eprescription.html", context = {
                    "medicationStatementIds": medication_statement_ids,
...
def patientCheck(request):
    ...
    return render(
            request, "patient_records/patient-check.html",
            {'patients': patients}
    )
...
def PatientList(request):
    ...
    if (Patient.objects.filter(aadhaarId = aadhaarId).exists()) and (
        ...
        # medication_items = MedicationItem.objects.values(
        #         'medication_statement_id').order_by('id')
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                obj.save()
                messages.success(
                        request, 'New ProblemList is successfully added.!'
                )
                # model = form.instance
                return redirect('/patient_records/patient-detail/{}'.format(id))
            except:
...
def patientVitalSignUpdate(request, id):
    ...
    form = VitalSignForm(
            initial = {'patient': patient, 'body_weight': vitalsign.body_weight,
                       ...
                       'blood_pressure_systolic': vitalsign
                           .blood_pressure_systolic,
                       'blood_pressure_diastolic': vitalsign
...
def medicationStatementCreate(request, id):
    ...
    if request.method == "POST":
        form = MedicationItemForm(
                request.POST, initial = {
                    'medication_statement': new_medication_statement_obj}
        )
        if form.is_valid():
            try:
                form.save()
                messages.success(
                        request,
                        'New Medication statement is successfully added.!'
                )
            except:
                raise ValueError
                messages.error(
                        request, 'failed to Add New medicaiton Statement list'
                )
            return redirect('/patient_records/patient-detail/{}'.format(id))
    else:
        form = MedicationItemForm(
    ...
    return render(
            request, 'patient_records/patient-medication-item.html',
            {'form': form, 'patient': patient}
    )
